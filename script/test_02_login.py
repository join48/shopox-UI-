...
class TestLogin:
    ...
    @pytest.mark.parametrize("title,phone,password,expect", read_json("login_success.json"))
    def test_01_login_scucess(self, title, phone, password, expect):
        self.current_title = title
        self.page.login(phone, password)
        result = self.page.get_success_result()
        log.debug(f"测试用例：{title} | 注册结果: {result} | 期望值: {expect}")
        assert  result in expect
    @pytest.mark.parametrize("title,phone,password,expect", read_json("login_fail.json"))
    def test_02_login_fail(self, title, phone, password, expect):
        self.current_title = title
        self.page.login(phone, password)
